...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCachetoken($cachename,$value,$expired){
        //TODO: set cache implementation
        $tokenmodel = D('token');
        $updata['token']=$value;
        $updata['addtime']=time()+$expired;
        $panduan = $tokenmodel->where(array('id'=>1))->save($updata);
        return false;
	 ...
	 * @return mixed
	 */
	protected function getCachetoken($cachename){
		//TODO: get cache implementation
        $tokenmodel = D('token');
        $addtimearr = $tokenmodel->where(array('id'=>1))->field('addtime,token')->find();

        $num = (int)$addtimearr['addtime'];
        if($num>time()){

            return $addtimearr['token'];
        }else {
            return false;
        }
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($rs = $this->getCachetoken($authname))  {
		...
		if ($result)
		{
			...
			$this->setCachetoken($authname,$this->access_token,$expire);
			return $this->access_token;
		}
		return false;
	}
